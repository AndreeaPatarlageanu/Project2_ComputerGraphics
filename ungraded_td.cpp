...
public:
  ~TriangleMesh() {}
    ...
    void Tutte(){
	std::map< std::set<int>>vertex_to_neighbors(vertices.size());
	...
	std::vector<std::std::vector<int>>vertex_to_neighbours(vertices.size());
	for( int i = 0; i < indices.size(); i++ ){
		for( auto it = vertex_to_neighbours_set[i].begin(); it != vertex... it++ ){
			vertex_to_neighbours[i].push_back(*it);
		}
	}
	std::vector<bool> boundary_vtx
	std::map<<std::pair<int, int>, std::vector<int> edge_to_traingles;
	for(int i= 0; i < indices.size(); i++ ){
	...
	for( auto it = edge_to_triangles.begin(); it != end; ++it ){
		if( it-> second.size() == 1){
	...
	while( current_vtx != starting_vtx ){
		...
		for( int i = 0; i < edges_on_the_boundary.size(); i++ ) {
			...
			else( i!= current_edge && edges_on_the_boundary[i].first == current_vtx ){
				current_vtx = edges_on_the_boundary[i].second;
				break;
			}
		}
	...
	parametrization.indices = indices;
	parametrization.vertices = vertices;
	
	for( int i = 0; i < boundary_chain.size(); i++ ){
		double angle = i * 2. * M_PI/ ( (double ) boundary_chain.size());
		double x = cos(angle);
		double y = sin(angle);
		parametrization.vertices[boundary_chain[i]] = Vector(x, y, 0);
	}
	...
	for( int it = 0; it < 1000; it ++ ) {
		...
		for( int i = 0; i < vertices.size(); i++ ) {
			if( !boundary_vtx[i]) {
				Vector avg(0,0,0);
				int num_nghb = vertex_to_neighbours[i].size();
				for( int j = 0; j < vertex_to_neighbours[i].size(); i++ ) {
					avg += parametrization.vertices[j];
				}
